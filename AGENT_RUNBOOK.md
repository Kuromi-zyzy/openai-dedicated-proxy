...
> **使命**：用户会给你一个 SSH 别名（或 `user@IP`）和部署档位（A / B / C）。你 SSH 上服务器，把档位对应的线路部署好、验收通过，然后产出客户端配置和凭据清单交付给用户。
...
- 档位 A：只需要 **VPS-1**（下称 `主机甲`）
- 档位 B/C：**VPS-1** 跑 WireGuard，**VPS-2**（下称 `主机乙`）跑 Xray（+HY2）。B/C 需要用户提供两个 SSH 目标。
...
## §5 档位 B：Xray REALITY（主机乙，TCP 443）
...
## §6 档位 C：Hysteria2（主机乙，UDP 443，与 Xray 共存）
