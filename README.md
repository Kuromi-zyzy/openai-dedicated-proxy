...
任意你已购买的 VPS 即可（Ubuntu 22.04+ / Debian 12，x86_64，1C/1G 起步绰绰有余）。本教程不展开选购；只有两点要求：

- 能开放 **TCP 22**（管理）、**一个自定义 UDP 端口**（WireGuard/Hysteria2 用）、**TCP 443**（Xray 用）——在你的云防火墙/安全组里放行；
- 想要多条线路容灾时，再准备第二台**不同地域**的 VPS（例如主力在日本，对照在新加坡/美国）。
...
| **B** | A + Xray REALITY 容灾线路 | 2 台（不同地域） | 主线路被干扰时需要备用出口 |
| **C** | B + Hysteria2 第三保险（与 REALITY 同机） | 2 台 | 需要抗丢包的 QUIC 兜底 |

**如果你懒：只部署档位 A 也完全成立**，随时可以补 B/C，它们共用同一个 Clash 策略组，故障时在界面点一下就切换。
...
B：第二台 VPS 装 Xray REALITY → 加节点进组 → 验收
C：B 同机装 Hysteria2 → 加节点进组 → 验收（同窗三线对照）
```
